...
(() => {
    const GM_getValue = (key, def) => {
        try {
            const raw = localStorage.getItem('ig_uvc_' + key);
            if (raw === null) return def;
            const p = JSON.parse(raw);
            return p === null || p === undefined ? def : p;
        } catch { return def; }
    };
    const GM_setValue = (key, val) => {
        try { localStorage.setItem('ig_uvc_' + key, JSON.stringify(val)); } catch {}
    };
...
(function () {
    'use strict';
    const strip = () => {
        document.querySelectorAll('._aagw').forEach((el) => el.remove());
    };
    strip();
...
Credits — modified by AlexRabbit (https://github.com/AlexRabbit)
  - Piknockyou — Ultimate Video Controls (Greasy Fork 563904)
  - ZhiYu (y252328) — Instagram Download Button (406535)
  - hdyzen — standard video controls (508188)
  - Naviamold — Fix Post Right Click (476010)
  - SN-Koarashi — Unlock Website Limit (404665, Global_Rabbit)
*/
